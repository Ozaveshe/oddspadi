...
export function AddToSlipButton({ match, prediction, compact = false }: { match: MatchSummary; prediction: PredictionSummary; compact?: boolean }) {
  ...
  if (!leg) return <span className={`slip-action-lock${compact ? " compact" : ""}`} role="status"><strong>{compact ? "Slip locked" : "Slip action locked"}</strong>{compact ? null : <span>Only a current, published value pick can be added.</span>}</span>;
