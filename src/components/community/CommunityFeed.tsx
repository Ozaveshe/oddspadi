...
export function CommunityFeed({ initialPosts, initialCursor, userId, matches }: { initialPosts: CommunityPost[]; initialCursor: string | null; userId: string | null; matches: ComposerMatch[] }) {
  ...
  return <div className="feed-list">
    {posts.map((post) => { const profile = author(post); const handle = profile?.username ?? "padi"; const liked = Boolean(post.likes?.some((like) => like.user_id === userId)); return (
      <article className="panel feed-post" key={post.id}>
        <div className="feed-post-head"><Link href={`/community/u/${encodeURIComponent(handle)}`}><strong>@{handle}</strong></Link>{profile?.display_name ? <span className="muted small">{profile.display_name}</span> : null}<span className="muted small" style={{ marginLeft: "auto" }}>{ago(post.created_at)}</span></div>
        ...
        <div className="feed-actions"><button type="button" className={liked ? "liked" : ""} aria-pressed={liked} onClick={() => toggleLike(post)}>♥ {post.likes?.length ?? 0}</button>{userId === post.author_id ? <button type="button" disabled={busy === post.id} onClick={() => remove(post.id)}>Delete</button> : null}</div>
      </article>
    ); })}
