...
export type LeagueStandingRow = { position: number; previousPosition: number | null; movement: number | null; teamId: string; teamName: string; played: number; wins: number; draws: number; losses: number; goalsFor: number; goalsAgainst: number; goalDifference: number; points: number; form: string };
...
export async function storedLeagueTable(slug: string, season: string): Promise<LeagueTable | null> {
  ...
  const teamIds = latest.map((row) => row.team_external_id); const { data: teams } = await db.from("op_teams").select("external_id,name").in("external_id", teamIds); const names = new Map((teams ?? []).map((team) => [team.external_id, team.name]));
  const rows = latest.map((row) => { const position = Number(row.position); const previousPosition = previous.get(row.team_external_id) ?? null; const gf = Number(row.goals_for ?? 0); const ga = Number(row.goals_against ?? 0); return { position, previousPosition, movement: previousPosition === null ? null : previousPosition - position, teamId: row.team_external_id, teamName: names.get(row.team_external_id) ?? String(row.team_external_id).replace(/^api-football:/, "Team "), played: Number(row.played ?? 0), wins: Number(row.wins ?? 0), draws: Number(row.draws ?? 0), losses: Number(row.losses ?? 0), goalsFor: gf, goalsAgainst: ga, goalDifference: gf - ga, points: Number(row.points ?? 0), form: formString(row.form) }; }).sort((a,b) => a.position-b.position);
