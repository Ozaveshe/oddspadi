...
export function buildEnginePerformanceWarnings({
  ...
  if (!ledgerAvailable) warnings.push({
    id: "ledger-unavailable",
    severity: "action",
    title: "Public outcome ledger is unavailable",
    detail: "Accuracy, ROI, calibration and closing-line metrics are withheld because the public-pick repository could not be read. Zero is not substituted for unavailable evidence."
  });
  if (ledgerAvailable && settledCount < 30) warnings.push({
    id: "small-sample",
    severity: "info",
  ...
  if (ledgerAvailable && settlement.totalPublicPicks > 0 && settlement.pendingRatio > 0.35) warnings.push({
  ...
  if (ledgerAvailable && roi !== null && roi < 0) warnings.push({
    id: "negative-roi",
    severity: "action",
  ...
  const badBucket = ledgerAvailable ? calibration.find((bucket) => bucket.predictions >= 5 && bucket.calibrationGap !== null && Math.abs(bucket.calibrationGap) >= 0.1) : null;
  ...
  if (ledgerAvailable && publicPickCount > 0 && qualityCoverage / publicPickCount < 0.8) warnings.push({
...
export function buildEnginePerformanceVerdict({
  ledgerAvailable,
  settledCount,
  blockingWarnings
}: {
  ...
  if (!ledgerAvailable) return {
    ...
    detail: "The public outcome ledger cannot be read in this runtime, so no performance claim is shown."
  };
  if (settledCount < 30) return {
    status: "early-evidence" as const,
    label: "Early evidence",
    detail: "The settled sample is measurable but too small for a durable quality claim."
  };
  if (blockingWarnings) return {
    status: "needs-attention" as const,
    label: "Needs attention",
    detail: "The engine is operating, but one or more outcome or settlement indicators require action."
  };
  return {
    status: "measurable" as const,
    label: "Measurable",
    detail: "The public sample is large enough to inspect. Continue monitoring calibration and ROI rather than treating either as permanent."
  };
}
  ...
  if (!client) {
    return {
      ...
      limitations: [
        ...
        "Player-level coverage cannot be verified from this runtime, so no corpus-depth claim is made."
      ]
    };
  }
...
export async function getEnginePerformanceReport() {
  ...
  const ledgerAvailable = ledger.source !== "unavailable";
  const warnings = buildEnginePerformanceWarnings({ settledCount: settled.length, settlement, providerStatus: daily.slate.provider.status, providerGapCount, roi: roi.roi, calibration, publicPickCount: publicRows.length, qualityCoverage, ledgerAvailable });

  const blockingWarnings = warnings.filter((warning) => warning.severity === "action").length;
  const verdict = buildEnginePerformanceVerdict({ ledgerAvailable, settledCount: settled.length, blockingWarnings });
...
export function formatEnginePerformanceCsv(report: EnginePerformanceReport): string {
  const publicEvidenceAvailable = report.source !== "unavailable";
  const publicValue = (value: unknown) => publicEvidenceAvailable ? value : "withheld: public ledger unavailable";
  const rows: unknown[][] = [
    ["OddsPadi engine performance export"],
    ["Generated at", report.generatedAt],
    ["Source", report.source],
    ["Source reason", report.sourceReason],
    ["Verdict", report.verdict.label],
    [],
    ["Public pick performance"],
    ["Settled picks", publicValue(report.publicPerformance.settledPicks)],
    ["Wins", publicValue(report.publicPerformance.wins)],
    ["Losses", publicValue(report.publicPerformance.losses)],
    ["Pushes", publicValue(report.publicPerformance.pushes)],
    ["Voids", publicValue(report.publicPerformance.voids)],
    ["Accuracy", publicValue(report.publicPerformance.accuracy)],
    ["ROI", publicValue(report.publicPerformance.roiSimulation.roi)],
    ["Brier score", publicValue(report.publicPerformance.brierScore)],
    [],
    ...
    ...(publicEvidenceAvailable
      ...
      : [["withheld: public ledger unavailable"]]),
    [],
