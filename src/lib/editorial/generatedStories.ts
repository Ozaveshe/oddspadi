...
export function generateEditorialStories(rows: EditorialOutcome[], now = new Date(), revisions: Partial<Record<GeneratedEditorialStory["generator"], number>> = {}): GeneratedEditorialStory[] {
  ...
  const editorialHorizon = now.getTime() + 7 * 86_400_000;
  const pending = rows.filter((row) => row.result === "pending" && row.kickoff_at && new Date(row.kickoff_at).getTime() >= now.getTime() && new Date(row.kickoff_at).getTime() < editorialHorizon).sort((a, b) => new Date(a.kickoff_at!).getTime() - new Date(b.kickoff_at!).getTime());
  const daily = distinctFixtures(pending).slice(0, 8);
  if (daily.length) stories.push({ ...base("daily-slate", date, daily, revisions["daily-slate"] ?? 1, now), title: `Matchday desk: ${daily.length} upcoming ${daily.length === 1 ? "fixture" : "fixtures"}`, excerpt: `The next ${daily.length} verified ${daily.length === 1 ? "fixture" : "fixtures"} on the OddsPadi board, with current model readings and abstentions kept honest.`, category: "Daily preview", body: [
