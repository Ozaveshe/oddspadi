...
export const POST = withApiHandler(async (request: Request) => {
  ...
  const failed = refresh.run.status === "failed" || refresh.run.status === "unavailable" || expiry.status === "unavailable";
  const partial = refresh.run.status === "partial" || expiry.status === "partial";
  return apiSuccess(
    { lookbackDays, refresh, expiry },
