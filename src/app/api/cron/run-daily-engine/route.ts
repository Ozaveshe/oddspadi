...
export const GET = withApiHandler(async () => apiSuccess(toPublicRunReceipt(await readLatestProviderRun(["run-daily-engine", "refresh-odds"]))));
...
export const POST = withApiHandler(async (request: Request) => {
  ...
  const result = await runDailyEngine({ sports: parsed.sports });
