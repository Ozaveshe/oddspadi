...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision context feature proof selector currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess(
    buildDecisionContextFeatureProofSelector({
      ladderReceipt,
      featureGap,
      fixtureFeatureReadiness
    }),
    { status: featureGap.status === "failed" ? 500 : 200 }
  );
}
