...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision provider key activation receipt currently supports football, basketball, and tennis.");
  }
  ...
  const contextProofReceipt = await observeDecisionContextFeatureProofReceipt({
    selector,
    runRequested,
    origin: url.origin,
    fetchImpl: fetch
  ...
  return apiSuccess(
    buildDecisionProviderKeyActivationReceipt({
      rehearsal,
      providerKeyPlan: context.providerActivationQueue.providerKeyPlan,
      env: process.env
    })
