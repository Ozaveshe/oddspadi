...
export async function GET(request: Request) {
  ...
  if (dryRun === "0" || dryRun?.toLowerCase() === "false") return apiError("football-data-market-segment-retest is read-only; use dryRun=1.", 400);
  ...
  const result = buildFootballDataMarketSegmentRetest({ benchmark, thresholdSweep });
  return apiSuccess(result, { status: benchmark.status === "failed" || thresholdSweep.status === "failed" ? 502 : 200 });
}
