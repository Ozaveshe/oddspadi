...
type GovernorSport = Extract<Sport, "football" | "basketball" | "tennis">;
...
async function buildModelCardInput({ date, sport, limit }: { date: string; sport: GovernorSport; limit: number }) {
...
export async function GET(request: Request) {
  ...
  const selectedSports = sports ?? (["football", "basketball", "tennis"] satisfies GovernorSport[]);

  const [readiness, corpusPlan, modelInputs] = await Promise.all([
    ...
    Promise.all(selectedSports.map((sport) => buildModelCardInput({ date, sport, limit: parseLimit(url.searchParams.get("limit")) })))
  ]);
  ...
  return apiSuccess(buildLearnedWeightPromotionGovernor({ date, shadowCandidates, modelCards }));
}
