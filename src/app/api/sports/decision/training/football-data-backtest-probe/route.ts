...
import { buildFootballDataCsvBacktestProbe } from "@/lib/sports/training/footballDataCsvBacktestProbe";
...
export async function GET(request: Request) {
  ...
  if (dryRun === "0" || dryRun?.toLowerCase() === "false") return apiError("football-data-backtest-probe is read-only; use dryRun=1.", 400);

  const result = await buildFootballDataCsvBacktestProbe({
