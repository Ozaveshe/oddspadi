...
export async function GET(request: Request) {
  ...
  if (dryRun === "0" || dryRun?.toLowerCase() === "false") return apiError("football-data-market-benchmark is read-only; use dryRun=1.", 400);

  const result = await buildFootballDataMarketBenchmark({
