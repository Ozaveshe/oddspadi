...
function parseLimit(value: string | null): number {
  ...
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 160) : 80;
}
...
export async function GET(request: Request) {
  ...
  return apiSuccess(buildDecisionMarketAuditMatrix({ date: query.date, slates, limit: parseLimit(url.searchParams.get("limit")) }));
}
