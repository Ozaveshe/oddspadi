...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Evidence acquisition planner currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess({
    ...context.evidenceAcquisitionPlanner,
    candidates: context.evidenceAcquisitionPlanner.candidates.slice(0, limit)
  });
}
