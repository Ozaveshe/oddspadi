...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision brain evidence debt resolver currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess({
    ...context.brainEvidenceDebtResolver,
    actions: context.brainEvidenceDebtResolver.actions.slice(0, limit)
  });
}
