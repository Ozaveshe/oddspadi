...
import { runDecisionAutonomousSettlement } from "@/lib/sports/prediction/decisionAutonomousSettlement";
import type { AutonomousSettlementSport } from "@/lib/sports/prediction/decisionAutonomousSettlement";
...
export async function POST(request: Request) {
  if (!isDecisionAdminAuthorized(request)) {
    return apiError("Autonomous settlement requires ODDSPADI_ADMIN_TOKEN and x-oddspadi-admin-token.", 401);
  }
  const sport = sportFrom(request);
  if (!sport) return apiError("sport must be football, basketball, or tennis.", 400);
  try {
