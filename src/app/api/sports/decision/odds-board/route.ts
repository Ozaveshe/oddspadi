...
function parseLimit(value: string | null): number {
  ...
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 80) : 20;
}
...
export async function GET(request: Request) {
  ...
  return apiSuccess(buildDecisionOddsBoard({ date: query.date, slates, limit: parseLimit(url.searchParams.get("limit")) }));
}
