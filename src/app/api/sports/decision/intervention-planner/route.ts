...
function parseLimit(value: string | null): number {
  ...
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 30) : 10;
}
...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision intervention planner currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess({
    ...context.interventionPlanner,
    scenarios: context.interventionPlanner.scenarios.slice(0, limit)
  });
}
