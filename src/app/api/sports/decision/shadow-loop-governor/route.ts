...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision shadow loop governor currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess(buildDecisionShadowLoopGovernor({ loop, memory, interpreter }));
}
