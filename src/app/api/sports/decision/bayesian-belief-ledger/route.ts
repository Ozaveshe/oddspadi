...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Bayesian belief ledger currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess({
    ...context.beliefLedger,
    beliefs: context.beliefLedger.beliefs.slice(0, limit)
  });
}
