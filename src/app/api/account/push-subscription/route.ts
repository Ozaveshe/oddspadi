...
async function authClient() { const supabase = await createSupabaseServerClient(); if (!supabase) return { response: privateJson({ error: "Push notifications are not configured." }, { status: 503 }) }; const { data: { user } } = await supabase.auth.getUser(); if (!user) return { response: privateJson({ error: "Sign in to manage notifications." }, { status: 401 }) }; return { supabase, user }; }
...
export async function POST(request: Request) {
  ...
  if (!count) return privateJson({ error: "Follow a team before enabling match alerts." }, { status: 400 });
  ...
  if (!isAllowedPushEndpoint(endpoint) || !isValidPushKey(p256dh, 40, 256) || !isValidPushKey(auth, 8, 128)) return privateJson({ error: "Invalid push subscription." }, { status: 400 });
  ...
  if (error) return databaseUnavailable("push subscription upsert", error, "Could not enable alerts right now."); return privateJson({ ok: true }, { status: 201 });
}
...
export async function DELETE(request: Request) {
  ...
  if (!isAllowedPushEndpoint(endpoint)) return privateJson({ error: "Missing subscription." }, { status: 400 });
  const { error } = await ctx.supabase.from("op_push_subscriptions").delete().eq("endpoint", endpoint).eq("user_id", ctx.user.id);
  if (error) return databaseUnavailable("push subscription delete", error, "Could not disable alerts right now."); return privateJson({ ok: true });
}
