...
export async function POST(request: Request) {
  ...
  if (!supabase) return privateJson({ error: "Profiles are not configured." }, { status: 503 });
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return privateJson({ error: "Sign in to update your profile." }, { status: 401 });
  ...
  if (displayName.length > 80) return privateJson({ error: "Display name must be 80 characters or fewer." }, { status: 400 });
  if (bio.length > 500) return privateJson({ error: "Bio must be 500 characters or fewer." }, { status: 400 });
  let favouriteTeam: string | null = null;
  if (favouriteTeamRequested && favouriteTeamId) {
    if (!isUuid(favouriteTeamId)) return privateJson({ error: "Choose a team from the search results." }, { status: 400 });
    ...
    if (!team) return privateJson({ error: "Choose a team from the search results." }, { status: 400 });
  ...
  return privateJson({ ok: true, favouriteTeam });
}
