...
export async function GET(request: Request) {
  ...
  if (!user) return privateJson({ poll, viewerChoice: null });
...
export async function POST(request: Request) {
  ...
  const supabase = await createSupabaseServerClient();
  if (!supabase) return privateJson({ error: "Community pulse is not enabled yet." }, { status: 503 });
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return privateJson({ error: "Sign in to vote." }, { status: 401 });
...
export async function DELETE(request: Request) {
  ...
  if (!supabase) return privateJson({ error: "Community pulse is not enabled yet." }, { status: 503 });
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return privateJson({ error: "Sign in to remove a vote." }, { status: 401 });
  const rateLimit = await enforceUserRateLimit(supabase, "community_poll_vote"); if (rateLimit) return rateLimit;
