...
describe("live OddsPadi product UI contract", () => {
  ...
  it("renders volatile tips surfaces per request so a cached provider failure can recover", () => {
    ...
    expect(health).toContain('checkFixtureAnalysisLinks(todayTips, "analysis links from today\'s tips")');
    expect(health).toContain('checkFixtureAnalysisLinks(weeklyTips, "analysis links from weekly radar")');
  });
