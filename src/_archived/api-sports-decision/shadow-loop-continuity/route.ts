...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision shadow loop continuity currently supports football, basketball, and tennis.");
  }
  ...
  const interpreter = buildDecisionShadowLoopReflectionReceiptInterpreter({
    reflection: context.shadowLoopReflection,
    receipt
  });

  return apiSuccess(buildDecisionShadowLoopContinuity({ interpreter }));
}
