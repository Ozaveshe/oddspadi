...
export async function GET(request: Request) {
  ...
  if (!isEnabled(url.searchParams.get("historical")) && !isEnabled(url.searchParams.get("publicHistory"))) {
    return apiSuccess(
      buildDecisionHistoricalDisciplineReceipt({
        date: query.date,
        sport: query.sport,
        marketPriorGovernor: context.marketPriorGovernor,
        marketCalibratedFusion: context.marketCalibratedFusion,
