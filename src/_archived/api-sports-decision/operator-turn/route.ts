...
export async function GET(request: Request) {
  ...
  const learningQueue = buildDecisionLearningQueue({ rows, date: query.date, sport: query.sport, readiness, memory, calibration, training });
  ...
  const researchAgent = buildDecisionResearchAgent({ rows, date: query.date, sport: query.sport, dataIntake, governance: modelGovernance, invalidationMonitor, autopilot });
  ...
  const activationAudit = buildDecisionActivationAudit({ date: query.date, sport: query.sport, readiness, dataIntake, governance: modelGovernance, autopilot, traceLedger });
  const proofRunner = buildDecisionProofRunner({ date: query.date, sport: query.sport, activationAudit, traceLedger, autopilot });
  const aiReviewLedger = buildDecisionAIReviewLedger({ date: query.date, sport: query.sport, orchestrator: aiOrchestrator, activationAudit, proofRunner });
  const counterfactualLab = buildDecisionCounterfactualLab({ rows, date: query.date, sport: query.sport, limit: 40 });
  const beliefRevision = buildDecisionBeliefRevision({ rows, date: query.date, sport: query.sport, counterfactualLab, proofRunner, aiReviewLedger, limit: 12 });
  ...
  const handoff = buildDecisionAIHandoffPacket({ rows, date: query.date, sport: query.sport, orchestrator: aiOrchestrator, aiReviewLedger, metacognition });
  const firewall = buildDecisionAIFirewall({ date: query.date, sport: query.sport, council: aiCouncil, orchestrator: aiOrchestrator, aiReviewLedger, metacognition, handoff });
  const citations = buildDecisionAICitationValidator({ date: query.date, sport: query.sport, handoff, firewall });
  const authority = buildDecisionAuthority({ rows, date: query.date, sport: query.sport, metacognition, handoff, firewall, proofRunner, aiReviewLedger });
  const kernel = buildDecisionAgentKernel({ date: query.date, sport: query.sport, metacognition, handoff, citations, firewall, authority, proofRunner, aiReviewLedger });
  const agentRuntime = buildDecisionAgentRuntime({ date: query.date, sport: query.sport, kernel, activationAudit, orchestrator: aiOrchestrator, autopilot, dataIntake, traceLedger });
  ...
  const contract = buildDecisionCapabilityContract({
    date: query.date,
    sport: query.sport,
    readiness,
    dataIntake,
