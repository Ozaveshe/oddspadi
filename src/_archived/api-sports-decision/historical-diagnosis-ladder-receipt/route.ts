...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision historical diagnosis ladder receipt currently supports football, basketball, and tennis.");
  }
  ...
  const receipt = await observeDecisionHistoricalDiagnosisLadderReceipt({
    ladder,
    runRequested,
    origin: url.origin,
    fetchImpl: fetch
