...
export async function GET(request: Request) {
  ...
  if (runRequested && !isDecisionAdminAuthorized(request)) {
    return apiError("OpenAI reasoning review requires ODDSPADI_ADMIN_TOKEN and x-oddspadi-admin-token.", 401);
  }
  ...
  if (!episode) return apiError("Unable to build operator episode before AI reasoning.", 502);

  return apiSuccess(await runDecisionAIReasoningGateway({ episode, runRequested }));
}
