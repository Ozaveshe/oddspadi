...
export async function GET(request: Request) {
  ...
  const includeBenchmark = isEnabled(url.searchParams.get("benchmark"));
  const includeBridge =
    isEnabled(url.searchParams.get("bridge")) || isEnabled(url.searchParams.get("historical")) || isEnabled(url.searchParams.get("publicHistory"));
  if (!includeBenchmark && !includeBridge) return apiSuccess(context.answerPromotionGate);
  ...
  const historicalProof = includeBridge
    ...
    : null;
  const benchmark = historicalProof?.benchmark ?? (includeBenchmark
  ...
  const promotionGate = buildDecisionAnswerPromotionGate({
    date: query.date,
    sport: query.sport,
  ...
  return apiSuccess(promotionGate);
}
