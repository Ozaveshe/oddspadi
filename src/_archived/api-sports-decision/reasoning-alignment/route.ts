...
function parseLimit(value: string | null): number {
  ...
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 40) : 10;
}
...
export async function GET(request: Request) {
  ...
  return apiSuccess(buildDecisionReasoningAlignment({ date: query.date, sport: query.sport, mind, informationGain }));
}
