...
type RunbookSport = Extract<Sport, "football" | "basketball" | "tennis">;
...
function parseLimit(value: string | null, fallback = 8): number {
  const parsed = Number(value);
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 40) : fallback;
}
...
async function buildSlate(date: string, sport: RunbookSport, limit: number) {
  ...
  return { sport, rows: rankedRows, matrix, governance, training, predictions: rankedRows.map((row) => row.prediction) };
}
...
export async function GET(request: Request) {
  ...
  const selectedSports = sports ?? (["football", "basketball", "tennis"] satisfies RunbookSport[]);
  const limit = parseLimit(url.searchParams.get("limit"));

  const [readiness, corpusPlan, slates] = await Promise.all([
    ...
    Promise.all(selectedSports.map((sport) => buildSlate(date, sport, limit)))
  ]);
  ...
  const modelCards = buildDecisionModelCards({
    date,
    inputs: slates.map((slate) => ({
      sport: slate.sport,
      matrix: slate.matrix,
      governance: slate.governance,
      training: slate.training,
      predictions: slate.predictions
    }))
  });
  const promotionGovernor = buildLearnedWeightPromotionGovernor({ date, shadowCandidates, modelCards });
  const oddsBoard = buildDecisionOddsBoard({
    date,
    slates: slates.map((slate) => ({ sport: slate.sport, rows: slate.rows })),
    limit: parseLimit(url.searchParams.get("boardLimit"), 80)
  });
  ...
  return apiSuccess(
    buildTrainingActivationRunbook({
      date,
      trainingCorpusProof,
      trainingReadiness,
      shadowCandidates,
