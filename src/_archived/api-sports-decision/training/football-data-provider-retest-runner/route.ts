...
export async function GET(request: Request) {
  ...
  if (dryRun === "0" || dryRun?.toLowerCase() === "false") return apiError("football-data-provider-retest-runner is read-only; use dryRun=1.", 400);
  ...
  return apiSuccess(runner, { status: benchmark.status === "failed" || thresholdSweep.status === "failed" ? 502 : 200 });
}
