...
export async function GET(request: Request) {
  ...
  if (dryRun === "0" || dryRun?.toLowerCase() === "false") return apiError("football-data-provider-retest-contract is read-only; use dryRun=1.", 400);
  ...
  return apiSuccess(contract, { status: benchmark.status === "failed" || thresholdSweep.status === "failed" ? 502 : 200 });
}
