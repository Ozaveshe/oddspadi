...
import { buildOddsEntitlementProbe } from "@/lib/sports/training/oddsEntitlementProbe";
...
export async function GET(request: Request) {
  ...
  return apiSuccess(probe, {
    status: runRequested && !probe.adminAuthorized ? 401 : probe.status === "provider-error" ? 502 : 200
  });
}
