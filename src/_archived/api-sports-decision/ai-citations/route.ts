...
export async function GET(request: Request) {
  ...
  if (runScope !== "none" && !isDecisionAdminAuthorized(request)) {
    return apiError("OpenAI citation execution requires ODDSPADI_ADMIN_TOKEN and x-oddspadi-admin-token.", 401);
  }
  ...
  const invalidationMonitor = buildDecisionInvalidationMonitor({ rows, date: query.date, sport: query.sport, dataIntake, governance, limit: 12 });
  ...
  const researchAgent = buildDecisionResearchAgent({ rows, date: query.date, sport: query.sport, dataIntake, governance, invalidationMonitor, autopilot });
  ...
  const activationAudit = buildDecisionActivationAudit({ date: query.date, sport: query.sport, readiness, dataIntake, governance, autopilot, traceLedger });
  ...
  return apiSuccess(buildDecisionAICitationValidator({ date: query.date, sport: query.sport, handoff, firewall }));
}
