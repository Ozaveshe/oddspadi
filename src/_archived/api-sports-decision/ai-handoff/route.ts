...
function parseEvidenceLimit(value: string | null): number {
  const parsed = Number(value);
  return Number.isInteger(parsed) && parsed > 0 ? Math.min(parsed, 80) : 36;
}
...
export async function GET(request: Request) {
  ...
  return apiSuccess(
    buildDecisionAIHandoffPacket({
      ...
      aiReviewLedger,
      metacognition,
      evidenceLimit: parseEvidenceLimit(url.searchParams.get("evidenceLimit"))
    })
  );
}
