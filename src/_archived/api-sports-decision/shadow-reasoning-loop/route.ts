...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision shadow reasoning loop currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess(buildDecisionShadowReasoningLoop({ memory, interpreter }));
}
