...
export async function GET(request: Request) {
  ...
  if (!DECISION_MULTI_SPORTS.includes(query.sport as DecisionMultiSport)) {
    return apiError("Decision shadow working memory currently supports football, basketball, and tennis.");
  }
  ...
  return apiSuccess(buildDecisionShadowWorkingMemory({ interpreter }));
}
