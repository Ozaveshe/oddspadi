...
- `GET /api/sports/decision/training/football-runtime-replay` executes a no-write football replay through the exact `modelFootballMatch` runtime entrypoint and returns a compact feature-contract, invocation-count, execution-hash, calibration, player-form coverage, duplicate-provider consolidation, and rejection summary. Cross-provider copies of the same real match are consolidated before chronology; API-Football is the identity spine when available so player rows remain joinable, while duplicate odds/context survive. Conflicting final scores fail closed. `POST` stores the same proved replay in `op_backtest_runs`. Both methods require `x-oddspadi-admin-token`; neither applies weights, promotes a model, publishes picks, or stakes. A completed benchmark run remains research evidence only: governance passes backtest parity only when the stored model-identity receipt proves the current runtime entrypoint and feature contract. Exact-runtime football learning also remains shadow-only until acceptable or strong chronology-safe player-form signals cover at least 60% of provider-capable fixtures in both the training and holdout windows.
- `npm run training:engine-preview -- --sport=football,basketball,tennis` replays the protected stored corpus without writing and prints both economic-selection and temperature-calibration receipts. It verifies the configured service credential belongs to OddsPadi before the first corpus query.
