...
- `GET /api/sports/decision/training/football-runtime-replay` executes a no-write football replay through the exact `modelFootballMatch` runtime entrypoint and returns a compact feature-contract, invocation-count, execution-hash, calibration, final-market-posterior, player-form coverage, duplicate-provider consolidation, and rejection summary. Cross-provider copies of the same real match are consolidated before chronology; API-Football is the identity spine when available so player rows remain joinable, while duplicate odds/context survive. Conflicting final scores fail closed. `POST` stores the same proved replay in `op_backtest_runs`. Both methods require `x-oddspadi-admin-token`; neither applies weights, promotes a model, publishes picks, or stakes. A completed benchmark run remains research evidence only: governance passes backtest parity only when the stored model-identity receipt proves the current runtime entrypoint and feature contract. Exact-runtime learning additionally requires a valid pre-match market-prior receipt; closing prices never calibrate the posterior. Football learning also remains shadow-only until acceptable or strong chronology-safe player-form signals cover at least 60% of provider-capable fixtures in both the training and holdout windows.
- `npm run training:engine-preview -- --sport=football,basketball,tennis` replays the protected stored corpus without writing and prints economic-selection, temperature-calibration, and final-market-posterior receipts. It verifies the configured service credential belongs to OddsPadi before the first corpus query.
...
- `src/lib/sports/training/footballRuntimeReplay.ts` - fail-closed football runtime replay. It rebuilds ordered pre-match form from earlier results only, shares daily league-strength and Elo-to-runtime-rating preprocessing, evaluates stored player availability and lineup freshness at the original kickoff clock, calls `modelFootballMatch`, applies frozen temperature scaling and the live no-vig market prior from a coherent pre-kickoff snapshot, records each scoring stage, and rejects rows whose identities, chronology, history floor, timestamps, or representable venue contract are incomplete.
