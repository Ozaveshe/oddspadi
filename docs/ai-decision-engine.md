...
- multiplies the heuristic blend by a model-bound scale learned only from an earlier priced training window; football thin-history evidence floors remain intact
...
- model-bound probability temperature, its fit/validation chronology, and untouched-holdout comparison
- model-bound market-prior scale, its nested priced fit/validation chronology, candidate-versus-identity proper scores, and effective live scale
- model-bound empirical value policy learned from strict earlier and recent final-posterior regimes, including per-regime 97.5% Wilson floors and a 95% joint-confidence floor by canonical 10-point bucket
- untouched-holdout point-estimate-versus-guarded pick comparison, with exact pick-count and ROI/yield receipt validation
- a second model-bound segment policy that repeats the same earlier/recent reliability test inside the exact football/basketball competition or tennis surface, plus a chained global-guarded-versus-segment-guarded holdout receipt
- final-posterior replay status, coherent pre-match odds coverage, average market-prior weight, and temperature-versus-posterior proper-score comparison
...
- apply a single temperature transform learned on the early training slice, accepted only when a later training-validation slice improves log loss without material Brier regression, and frozen before holdout
- learn the market-prior scale only after temperature calibration, fit it on the early half of the later priced training slice, accept it only on that slice's untouched later half using the same log-loss/Brier gate, and freeze it before outer holdout
- require both edge and EV to remain positive at the minimum of the aggregate 95% floor and the earlier/recent 97.5% floors for the selected final-posterior probability bucket; thin, unsplittable, or regime-unstable buckets abstain
- freeze that empirical policy before outer holdout, record how many point-estimate picks it removed, and reject promotion when the policy or comparison receipt is malformed
- require the exact live competition or surface to have its own eligible bucket with at least 20 observations in each regime and 40 total; unresolved, unseen, thin, or locally unstable segments abstain even when pooled sport-wide evidence is favorable
- prove that holdout rows then traversed the live no-vig market-prior stage using coherent decision-time quotes; closing and post-kickoff prices cannot satisfy this receipt
...
- `GET /api/sports/decision/training/calibration-promotion?sport=football|basketball|tennis` returns the active model-bound promotion plus its current out-of-sample drift receipt. Production learned weights remain inactive unless the receipt exactly matches the promotion identity and passes sample, freshness, Brier, log-loss, expected-calibration-error, earlier/recent chronology, and probability-population-shift gates.
- `GET /api/sports/decision/training/champion-challenger?sport=football|basketball|tennis&challengerCandidateId=<id>` previews a no-write paired comparison against the one active sport champion. Authenticated `POST` stores the immutable receipt. It requires exact fixture/market/selection pairs, 60 paired settlements, 80% coverage on both sides, strict 30/30 chronological regimes, paired Brier/log-loss non-inferiority, statistically supported superiority on at least one primary score, calibration safety, and current evidence.
- `POST /api/sports/decision/training/calibration-promotion` accepts `comparisonReceiptId` for a distinct challenger. The database atomically validates that the receipt is fresh, promotable, and bound to the current champion and exact candidate before revoking the champion and inserting its replacement. Bootstrap and strictly later same-identity calibration refreshes are explicit non-challenger paths; none auto-promotes.
...
- `GET /api/sports/decision/training/football-runtime-replay` executes a no-write football replay through the exact `modelFootballMatch` runtime entrypoint and returns a compact feature-contract, invocation-count, execution-hash, temperature policy, market-prior scaling policy, global and exact-competition empirical value-floor policies with chained holdout comparisons, final-market-posterior, player-form coverage, duplicate-provider consolidation, and rejection summary. Cross-provider copies of the same real match are consolidated before chronology; API-Football is the identity spine when available so player rows remain joinable, while duplicate odds/context survive. Conflicting final scores and unsplittable kickoff cohorts fail closed. `POST` stores the same proved replay in `op_backtest_runs`. Both methods require `x-oddspadi-admin-token`; neither applies weights, promotes a model, publishes picks, or stakes. A completed benchmark run remains research evidence only: governance passes backtest parity only when the stored model-identity receipt proves the current runtime entrypoint and feature contract. Exact-runtime learning additionally requires valid training-only temperature, market-prior scaling, global empirical-value, and segment-value policies, internally consistent chained holdout-removal comparisons, plus a pre-match market-prior replay receipt; closing prices never calibrate the posterior. Football learning also remains shadow-only until acceptable or strong chronology-safe player-form signals cover at least 60% of provider-capable fixtures in both the training and holdout windows.
- `npm run training:engine-preview -- --sport=football,basketball,tennis` replays the protected stored corpus without writing and prints economic-selection, temperature-calibration, market-prior-scaling, temporal empirical-value-floor, exact-segment reliability, regime-drift, chained holdout-removal, and final-market-posterior receipts. It verifies the configured service credential belongs to OddsPadi before the first corpus query.
...
- `POST /api/sports/decision/training/provider-sync` fetches provider data and maps it into the normalized ingestion layer. It supports `provider=api-football` for fixture history, an optional numeric `team` filter for bounded promoted-team or coverage-gap recovery, optional `includeEvents=1` API-Football event archives, optional `includeNews=1` NewsAPI archives, optional `includeContext=1` standings/injuries/suspensions/lineups/weather archives, and explicit `includePlayerStats=1` finished-match player-performance ingestion. Player-performance responses separate fetched, normalized, fixture-requested, fixture-covered, stored, and readback-verified counts. A fixture is covered only when both teams have at least 11 participants with recorded minutes; incomplete player payloads return `invalid-response` and are not stored. Daily inference and runtime replay use only performances from kickoffs strictly earlier than the prediction clock. It also supports `provider=api-basketball` for basketball game history, `provider=api-tennis` for tennis event history, and `provider=the-odds-api` for historical h2h odds snapshots. It defaults to `dryRun`; pass `dryRun=0` only after reviewing the normalized counts and provider budget.
- `npm run player-history:prepare -- --from=YYYY-MM-DD --to=YYYY-MM-DD --max-fixtures=N [--league=ID] [--team=ID]` is the service-key-free recovery lane for historical API-Football player rows. It reuses the production fixture/player normalizers, requires at least 11 minute-bearing players per team, records provider quota before and after the bounded run, paces player requests below the provider's per-minute ceiling, and emits checksummed, idempotent SQL chunks under `output/player-history/`. The optional numeric team ID narrows the fixture pull without weakening the two-team completeness check. If the provider rejects a partial run, pass its `manifest.json` back with `--retry-manifest=...` to request exactly the failed fixture IDs. The command does not connect to Supabase or apply its artifacts; a named OddsPadi database operator must review the manifest and execute the chunks against project `wncwtzqipnoqwmqlznqn`, followed by corpus and duplicate-key readback checks.
...
- `src/lib/sports/prediction/calibrationDriftGuard.ts` recomputes exact-identity, out-of-sample live calibration and distribution-shift evidence before a promoted profile can activate.
- `src/lib/sports/prediction/championChallenger.ts` performs paired proper-score, coverage, freshness, and chronology tests before a distinct model can become promotion-eligible.
- `src/lib/sports/prediction/championChallengerRepository.ts` previews and stores immutable comparison receipts and leaves the final decision behind the admin-gated atomic promotion function.
...
- `src/lib/sports/training/footballRuntimeReplay.ts` - fail-closed football runtime replay. It rebuilds ordered pre-match form from earlier results only, shares daily league-strength and Elo-to-runtime-rating preprocessing, evaluates stored player availability and lineup freshness at the original kickoff clock, calls `modelFootballMatch`, applies frozen temperature and prospectively validated market-prior scaling before the live no-vig market posterior from a coherent pre-kickoff snapshot, records each scoring stage, and rejects rows whose identities, chronology, history floor, timestamps, or representable venue contract are incomplete.
