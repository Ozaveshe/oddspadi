...
- `npm run player-history:prepare -- --from=YYYY-MM-DD --to=YYYY-MM-DD --max-fixtures=N` is the service-key-free recovery lane for historical API-Football player rows. It reuses the production fixture/player normalizers, requires at least 11 minute-bearing players per team, records provider quota before and after the bounded run, paces player requests below the provider's per-minute ceiling, and emits checksummed, idempotent SQL chunks under `output/player-history/`. If the provider rejects a partial run, pass its `manifest.json` back with `--retry-manifest=...` to request exactly the failed fixture IDs. The command does not connect to Supabase or apply its artifacts; a named OddsPadi database operator must review the manifest and execute the chunks against project `wncwtzqipnoqwmqlznqn`, followed by corpus and duplicate-key readback checks.
