...
| `model-learning-sweep` | `45 4 * * *` | Claims the global sports-pipeline lock, stores per-sport calibration daily, evaluates every newly stored distinct candidate against the active champion with immutable paired receipts, bootstraps missing or stale exact runtime-entrypoint evidence, and refreshes every sport each Monday without auto-promotion |
