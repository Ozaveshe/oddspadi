...
export default async function modelLearningSweep(_request: Request, context: Context) {
  ...
  try {
    ...
    // straight through: a pass-through forwards every upstream header verbatim
    // into the scheduled-function reply. The other sweeps in this directory
    // already do it this way.
    const response = await fetch(new URL("/.netlify/functions/model-learning-worker-background", siteUrl), {
      method: "POST",
