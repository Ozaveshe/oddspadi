...
export default async function outcomeLedgerWorker(request: Request, context: Context): Promise<Response> {
  ...
  if (!siteUrl || !token) return Response.json({ success: false, error: "Outcome ledger worker configuration is incomplete." }, { status: 503 });
  ...
  try {
    const endpoint = new URL("/api/cron/outcome-ledger", siteUrl);
    endpoint.searchParams.set("days", clean(Netlify.env.get("ODDSPADI_OUTCOME_LEDGER_DAYS")) ?? "14");
    ...
    return Response.json({ success: false, error: error instanceof Error ? error.message : "Outcome ledger run failed." }, { status: 504 });
  }
}
