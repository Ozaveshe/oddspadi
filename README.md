...
Use `POST /api/sports/decision/training/provider-sync` to fetch and normalize the first real provider feeds. It supports `provider=api-football` for finished football fixture history, optional `includeEvents=1` event archives, optional `includeNews=1` NewsAPI archives, optional `includeContext=1` standings/injuries/suspensions/lineups/weather archives, `provider=api-basketball` for basketball game history, `provider=api-tennis` for tennis event history, and `provider=the-odds-api` for historical h2h odds snapshots. It also defaults to `dryRun`; pass `dryRun=0` only when the payload looks correct.
...
- `/api/sports/decision/training/provider-sync?provider=api-football&league=39&season=2025&date=2025-08-01&includeEvents=1&includeNews=1&includeContext=1` dry-runs one API-Football fixture import with match-event, news, standings, availability, lineup, and weather snapshots. `/api/sports/decision/training/provider-sync?provider=api-basketball&league=12&season=2025&date=2025-01-15` dry-runs basketball games. `/api/sports/decision/training/provider-sync?provider=api-tennis&date=2025-06-03` dry-runs tennis events. `/api/sports/decision/training/provider-sync?provider=the-odds-api&sportKey=soccer_epl&date=2025-08-01T12:00:00Z` dry-runs one historical odds snapshot import.
