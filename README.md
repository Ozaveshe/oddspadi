...
Historical learning is wired through `src/lib/sports/prediction/decisionLearningProfile.ts`. Exact-runtime replays learn a one-parameter probability temperature on the early chronological training slice, accept it only on a later validation slice, freeze it before holdout, and then replay the same bounded no-vig market posterior used by live predictions. Only coherent decision-time prices may enter that posterior; explicit closing prices remain evaluation evidence and post-kickoff prices fail closed. Stored receipts separate raw-model versus temperature scores from temperature versus final-posterior scores, coverage, margin, and blend weight. The live decision engine can consume learned guardrails only after promotion verifies all three stages. Demo-seed backtests remain pipeline proof only. Use `npm run training:engine-preview -- --sport=all` for a no-write stored-corpus receipt; it refuses a service credential from any project other than OddsPadi before querying.
