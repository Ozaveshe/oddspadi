...
Historical learning is wired through `src/lib/sports/prediction/decisionLearningProfile.ts`. Exact-runtime replays now learn a one-parameter probability temperature on the early chronological training slice, accept it only on a later validation slice, freeze it before holdout, and store a baseline-versus-calibrated proper-score receipt. The live decision engine can consume that model-bound transform plus value-edge weight, data-quality weight, market-adjustment weight, and minimum-edge abstention gates only after the separate promotion gates pass. Demo-seed backtests are visible as pipeline proof but are never applied to live guardrails. Use `npm run training:engine-preview -- --sport=all` for a no-write stored-corpus receipt; it refuses a service credential from any project other than OddsPadi before querying.
